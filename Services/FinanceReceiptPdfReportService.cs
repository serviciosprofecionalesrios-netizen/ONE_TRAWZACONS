...
{
    public static class FinanceReceiptPdfReportService
    {
        public static byte[] GeneratePdf(FinanceReceipt receipt, string webRootPath, string? signUrl)
        {
            QuestPDF.Settings.License = LicenseType.Community;
            var logoBytes = TryLoadLogo(webRootPath);
            var qrBytes = string.IsNullOrWhiteSpace(signUrl) ? null : BuildQrCode(signUrl);
            var signatureBytes = ExtractSignaturePng(receipt.SignatureDataUrl);
            ...
            return Document.Create(container =>
            {
                container.Page(page =>
                {
                    ...
                    page.Header().Element(h => ComposeHeader(h, receipt, logoBytes));
                    page.Content().Element(c => ComposeContent(c, receipt, signUrl, qrBytes, signatureBytes));
                    page.Footer().AlignRight().Text($"Generado {DateTime.Now:dd/MM/yyyy HH:mm}").FontSize(8).FontColor(Colors.Grey.Darken1);
                });
            }).GeneratePdf();
        }

        private static void ComposeHeader(IContainer container, FinanceReceipt receipt, byte[]? logoBytes)
        {
            container.BorderBottom(2)
                ...
                .Row(row =>
                {
                    ...
                    {
                        col.Item().Text("Finanzas - Formulario de Recibo").SemiBold().FontSize(14).FontColor("#11439A");
                        col.Item().Text($"No. Recibo: {receipt.ReceiptNumber}").FontSize(10).FontColor(Colors.Grey.Darken2);
                    });

                    row.ConstantItem(180).AlignRight().Column(col =>
                    {
                        col.Item().Text("Estado de firma").FontSize(8).FontColor(Colors.Grey.Darken1);
                        if (receipt.SignedAtUtc.HasValue)
                        {
                            col.Item().Text("Firmado electronicamente").FontSize(9).SemiBold().FontColor(Colors.Green.Darken2);
                            col.Item().Text($"Por: {receipt.SignedByName}").FontSize(9);
                            col.Item().Text($"{receipt.SignedAtUtc.Value.ToLocalTime():dd/MM/yyyy HH:mm}").FontSize(9);
                        }
                        else
                        {
                            col.Item().Text("Pendiente de firma").FontSize(9).SemiBold().FontColor(Colors.Orange.Darken2);
                        }
                    });
                });
        }
            ...
            byte[]? signatureBytes)
        {
            container.PaddingTop(14).Column(col =>
            {
                ...
                col.Item().Border(1).BorderColor(Colors.Grey.Lighten2).Padding(10).Table(table =>
                {
                    ...
                    AddRow(table, "No. Recibo", receipt.ReceiptNumber);
                    AddRow(table, "Sitio / Sede", receipt.Site);
                    AddRow(table, "Recibido de", receipt.ReceivedFrom);
                    AddRow(table, "Monto", $"{receipt.Currency} {receipt.Amount.ToString("N2", CultureInfo.InvariantCulture)}");
                    AddRow(table, "Fecha recibo", receipt.ReceiptDateUtc.ToLocalTime().ToString("dd/MM/yyyy", CultureInfo.InvariantCulture));
                });
                ...
                col.Item().Border(1).BorderColor(Colors.Grey.Lighten2).Padding(10).Column(block =>
                {
                    ...
                    block.Item().PaddingTop(6).Text(receipt.Concept);
                });
                ...
                if (!string.IsNullOrWhiteSpace(receipt.Notes))
                {
                    col.Item().Border(1).BorderColor(Colors.Grey.Lighten2).Padding(10).Column(block =>
                    {
                        ...
                        block.Item().PaddingTop(6).Text(receipt.Notes!);
                    });
                }

                col.Item().Border(1).BorderColor(Colors.Grey.Lighten2).Padding(10).Column(block =>
                {
                    block.Item().Text("Firma del recibido").SemiBold().FontColor(Colors.Grey.Darken4);

                    if (signatureBytes != null)
                    {
                        block.Item().PaddingTop(8).Height(110).AlignLeft().Image(signatureBytes).FitHeight();
                        block.Item().PaddingTop(4).Text($"Firmado por {receipt.SignedByName} el {receipt.SignedAtUtc?.ToLocalTime():dd/MM/yyyy HH:mm}")
                            .FontSize(9)
                ...
                if (!receipt.SignedAtUtc.HasValue && !string.IsNullOrWhiteSpace(signUrl))
                {
